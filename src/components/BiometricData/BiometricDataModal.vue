<template>
  <Dialog v-model:visible="visible" modal draggable :header="recommendationGenerated ? 'Tu Recomendación' : 'Nueva medición'" class="w-1/2">
    <div v-if="!recommendationGenerated">
...
const id = ref(null);
const user = ref([]);
const recommendationGenerated = ref(false);
...
const beforeOpen = async () => {
  ...
  recommendationGenerated.value = false;
  itemData.value = {
    heart_frequency:null,
    steps:null,
    distance:null,
  ...
  await generateRecommendation(itemData.value)
    .then(response => {
      ...
      toast.add({
        severity: 'success',
        summary: 'Éxito',
        detail: 'Recomendacion generada correctamente',
        life: 3000,
      });
      recommendation.value = response.data.recommendation;
      recommendationGenerated.value = true;
